...
class DatabaseStorage:
    ...
    async def save_products_batch_db(self, products: List[Product]) -> int:
        ...
        for product in products:
            if not product.id or not product.url:
                continue
    ...
    async def save_creators_batch_db(self, creators: List[Creator]) -> int:
        ...
        """
        if not self.is_available() or not creators:
            return 0

        # Filter out invalid creators
        valid_creators = [c for c in creators if c.username]
        ...
        try:
            ...
            logger.error(
                "creators_batch_db_save_error",
                count=len(valid_creators),
                error=str(e),
                error_type=type(e).__name__,
            )
            return 0
            ...
            logger.error(
                "creators_batch_db_save_unexpected_error",
                count=len(valid_creators),
                error=str(e),
                error_type=type(e).__name__,
            )
