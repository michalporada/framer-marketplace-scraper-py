...
# Skrypt do uruchomienia GitHub Actions workflow
...
if [ -z "$GITHUB_TOKEN" ]; then
    ...
    echo "6. Następnie uruchom ponownie: ./run_workflow.sh"
    echo ""
    echo "Lub użyj:"
    echo "  GITHUB_TOKEN=twoj_token ./run_workflow.sh"
    exit 1
fi
...
echo "🚀 Uruchamianie workflow (ID: ${WORKFLOW_ID})..."
...
if [ "$HTTP_CODE" == "204" ]; then
    echo "✅ Workflow uruchomiony pomyślnie!"
    echo ""
    echo "🔗 Sprawdź status:"
